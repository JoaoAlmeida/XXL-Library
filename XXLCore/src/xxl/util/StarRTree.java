...
public class StarRTree extends RTree {
    ...
    public void open() throws IOException, ClassNotFoundException {
        ...
        if (resourceFile.exists()) {
            ...
            IndexEntry root = (IndexEntry) ((IndexEntry) this.createIndexEntry(height)).initialize(rootMBR).initialize(rootPageId);
    ...
    public void flush() throws IOException {
        ...
        if (this.rootEntry() != null) {
            ...
            //Inserido por mim!
            //rootDescriptor = this.rootDescriptor;
    ...
    public String toString(IndexEntry n, int level) {
        ...
        if (n.level() == 0) {
            for (Iterator it = n.get().entries(); it.hasNext();) {
                ...
                DoublePointRectangle point = (DoublePointRectangle) dataEntry.getData();
                point.getCorner(false).getValue(1);

                str.append(toString((DoublePointRectangle) dataEntry.getData()));
                str.append('\n');
            }

            //Se não estiver na folha, desce mais na arvore em busca da folha
        } else {
            if ((n.level() - 1) >= level) {
...
//            System.out.println("I = " + i);
           // System.out.println("Values[ " + d + " ]" + " = " + values[d]);
            d++;
        }
    ...
    public void acessaRoot(MaxDoubleRTree rtree, LinkedList lista) {

        IndexEntry n = (IndexEntry) rtree.rootEntry();
        
        int level = 0;

        acessaRoot(n, level, lista);
    }

    //preenche lista com KPES. Os KPES contém os objetos espaciais com seus scores
    public void acessaRoot(IndexEntry n, int level, LinkedList<KPE> lista) {

        Descriptor desc = rootDescriptor;
        
        if (n.level() == 0) {
            for (Iterator it = n.get().descriptors(descriptor(n)); it.hasNext();) { //pegando descriptors ao inves de get.entries. Descriptors podem ser convertidos para retangulos
                              
                MaxDoubleRectangle dataEntry = (MaxDoubleRectangle) it.next();
               
                System.out.println("Data Entry " + dataEntry);
                System.out.println("Score: " + dataEntry.getScore());
...
//                point.getCorner(false).getValue(1);                                                         
            ...
            if ((n.level() - 1) >= level) {
                for (Iterator it = n.get().entries(); it.hasNext();) {
                    acessaRoot((IndexEntry) it.next(), level, lista);
                }
            }
        }
...
//                Integer.parseInt(properties.getProperty("rtree.maxNodeEntries")));
        ...
        MaxDoubleRTree aggTree = new MaxDoubleRTree(statisticCenter, "",
                properties.getProperty("experiment.dataSetFolder") + "/rtree",
                Integer.parseInt(properties.getProperty("rtree.dimensions")),
                Integer.parseInt(properties.getProperty("rtree.bufferSize")),
                Integer.parseInt(properties.getProperty("diskStorage.blockSize")),
        ...
        BufferedReader input = new BufferedReader(new InputStreamReader(new FileInputStream("novoIndex.txt")));

        String line = input.readLine();
        double[] point;
        for (int i = 1; line != null; i++) {
            if (!line.startsWith("#")) {
                point = getValues(line, 3);
                ...
                MaxDoubleRectangle mbr = new MaxDoubleRectangle(aux, aux, score);

                //rTree.insert(new KPE(mbr, i, IntegerConverter.DEFAULT_INSTANCE));
                aggTree.insert(new KPE(mbr, i, IntegerConverter.DEFAULT_INSTANCE));
            }
        ...
        aggTree.acessaRoot(aggTree, lista);

//        double[] from = new double[]{0, 0};
//        double[] to = new double[]{4, 7};
